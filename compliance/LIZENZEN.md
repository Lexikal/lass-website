...
| ~~`assets/praxis.mp4` (+ `assets/praxis-poster.jpg`), 2026-08-21 bis 2026-08-22~~ | **Vom Kunden als Fehleinsatz zurückgezogen (2026-08-22)** — Dateien gelöscht, Einbindung in `leistungen.html` #praxis rückgängig gemacht. War KI-generiert (Hand in Einweghandschuh desinfiziert eine Praxisliege), Quelle unbekannt, keine Kennzeichnung — Details bleiben in `OFFENE-PUNKTE.md` Punkt 11 dokumentiert, nicht gelöscht. |  |  |  |  |
| `assets/leistungen-bg.mp4` (+ `assets/leistungen-bg-poster.jpg`) | Vom Kunden am 2026-08-22 bereitgestellte Datei (`..._generated_video.MP4`, Dateiname deutet auf KI-Generierungstool hin) — **welches Tool erzeugt hat, ist nicht bekannt** | Unbekannt — **Nutzungsbedingungen des Erzeuger-Tools nicht geprüft** | Ungeklärt, bis Quelle feststeht | ❌ nicht vorhanden | **Ja — zeigt eine Hand in Einweghandschuh, die verschiedene Kontaktflächen desinfiziert. Läuft als fixierter Vollbild-Hintergrund über die GANZE Seite `leistungen.html` (alle sechs Leistungen), nicht nur einen Abschnitt — größere Reichweite als alle bisherigen Vorkommen. Gleicher Grundkonflikt wie Punkt 10/11 (siehe `OFFENE-PUNKTE.md` Punkt 12), diesmal ausdrücklich auf Kundenwunsch trotz bekanntem Konflikt beibehalten statt durch Alternative ersetzt.** |
